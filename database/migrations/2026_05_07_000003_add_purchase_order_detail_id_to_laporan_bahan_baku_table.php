<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('laporan_bahan_baku', function (Blueprint $table) {
            $table->foreignId('purchase_order_detail_id')
                ->nullable()
                ->after('estimasi_tanggal_bayar')
                ->constrained('purchase_order_detail')
                ->nullOnDelete();

            $table->unique('purchase_order_detail_id', 'laporan_bahan_baku_po_detail_unique');
        });
    }

    public function down(): void
    {
        Schema::table('laporan_bahan_baku', function (Blueprint $table) {
            $table->dropUnique('laporan_bahan_baku_po_detail_unique');
            $table->dropConstrainedForeignId('purchase_order_detail_id');
        });
    }
};
